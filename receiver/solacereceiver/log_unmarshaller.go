...
func (u *solaceLogsUnmarshaller) unmarshalToLogData(message *inboundMessage) (*logs_v1.LogRecord, error) {
	var data = message.GetData()
	if len(data) == 0 {
		return nil, errEmptyPayload
	}
	var logRecord logs_v1.LogRecord
...
func (u *solaceLogsUnmarshaller) populateLogs(logData *logs_v1.LogRecord, logs plog.Logs) {
	...
	if strVal, ok := logData.Body.Value.(*logs_v1.AnyValue_StringValue); ok {
		logRecord.Body().SetStr(strVal.StringValue)
	}
	for _, attr := range logData.Attributes {
		if attr.Key == "solace.broker.name" {
			logRecord.Attributes().PutStr("host.name", attr.Value.GetStringValue())
		} else if attr.Key == "solace.broker.message-vpn" {
			logRecord.Attributes().PutStr("service", attr.Value.GetStringValue())
		}
		switch casted := attr.Value.Value.(type) {
		case *logs_v1.AnyValue_StringValue:
			logRecord.Attributes().PutStr(attr.Key, casted.StringValue)
		case *logs_v1.AnyValue_BoolValue:
			logRecord.Attributes().PutBool(attr.Key, casted.BoolValue)
		case *logs_v1.AnyValue_IntValue:
			logRecord.Attributes().PutInt(attr.Key, casted.IntValue)
		case *logs_v1.AnyValue_DoubleValue:
			logRecord.Attributes().PutDouble(attr.Key, casted.DoubleValue)
		default:
			u.logger.Warn("Unknown attribute type", zap.String("type", fmt.Sprintf("%t", attr.Value.Value)))
			logRecord.Attributes().PutEmpty(attr.Key)
		}
	}
}
